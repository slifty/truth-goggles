(function() {
	...
	for(var x = 0; x < paragraphs.length ; ++x) {
		...
		script.src = criticalDomain + "/api/snippets/" + '?jsonp=1&oid=' + x + '&context=' + encodeURIComponent(text);
		document.getElementsByTagName('head')[0].appendChild(script);
	}
	
	window.goggles_get_snippets_callback = function(data) {
		for(var x = 0; x < data.snippets.length; ++x) {
			var snippetStart = data.snippets[x].snippet_start;
			var snippetEnd = snippetStart + data.snippets[x].snippet_length;
    		var snippet = data.snippets[x].snippet;
